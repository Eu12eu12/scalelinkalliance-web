...
// Base empty fallback; all packages are populated dynamically from /api/cms/services
...
const RequestServicePage = () => {
  ...
  useEffect(() => {
    ...
    let serviceName = SLUG_TO_SERVICE_NAME[serviceSlug] || SLUG_TO_SERVICE_NAME[requestedSlug];
    if (!serviceName) {
      for (const cat of Object.values(SERVICE_CATEGORIES)) {
        const found = cat.services.find(s => 
          s.toLowerCase() === serviceSlug.toLowerCase() ||
          getServiceSlug(s) === serviceSlug.toLowerCase()
        );
        if (found) {
          serviceName = found;
          break;
    ...
    if (!serviceName) {
      const foundInPkg = Object.keys(servicesWithPackages).find(k =>
        k.toLowerCase() === serviceSlug.toLowerCase() ||
        getServiceSlug(k) === serviceSlug.toLowerCase()
      );
      if (foundInPkg) serviceName = foundInPkg;
    }
    if (!serviceName) serviceName = serviceSlug;

    // 2. Resolve package key
    const serviceData = servicesWithPackages[serviceName] || servicesWithPackages[serviceSlug];
    const availablePackages = serviceData?.packages ? Object.keys(serviceData.packages) : [];

    let packageKey = (pkgParam || 'starter').toLowerCase();
    if (availablePackages.length > 0) {
      if (availablePackages.includes(packageKey)) {
        // exact match
      } else if (packageKey === 'standard') {
        packageKey = availablePackages.includes('growth') ? 'growth' : 'starter';
      } else if (packageKey === 'basic') {
        packageKey = availablePackages.includes('starter') ? 'starter' : 'starter';
      } else if (availablePackages.includes('starter')) {
        packageKey = 'starter';
      } else {
        packageKey = availablePackages[0];
      }
    }
